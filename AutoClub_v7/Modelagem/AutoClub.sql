CREATE SCHEMA IF NOT EXISTS `autoclub` DEFAULT CHARACTER SET utf8mb3 ;
USE `autoclub` ;
...
CREATE TABLE IF NOT EXISTS `autoclub`.`pecas` (
  ...
  PRIMARY KEY (`id_pecas`, `id_autopecas`),
...
INSERT INTO adm(id_adm, nome, email, senha, foto, nivel)
...
INSERT INTO pecas(id_pecas, nome, preco, id_autopecas)
VALUES(default, "Filtro de Óleo", 140.00, 1),
	  (default, "Radiador", 200.00, 2),
	  (default, "Bateria", 170.00, 3);
...
SELECT c.marca, c.modelo, c.placa, u.nome, u.id_usuario
FROM carro as c
INNER JOIN usuario as u on c.id_usuario = u.id_usuario;
-- senha do Caio 123ABC@
SELECT * FROM carro WHERE id_usuario = 5;
